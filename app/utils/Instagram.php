...
class Instagram {
  ...
  public function getPublicMedia($hashtag, $id = 'self', $limit = 0) {
      //Log::error("PUBLIC");
      return $this->_makeCall('tags/'.$hashtag.'/media/recent', ($id === 'self'), array('count' => $limit));
  }
  ...
  protected function _makeCall($function, $auth = false, $params = null, $method = 'GET') {
      //Log::error( "<br>current token = ".$this->getAccessToken());
    if (false === $auth) {
    ...
    curl_close($ch);
    //Log::error("[_makeCall.jsonData] = ".$jsonData);
